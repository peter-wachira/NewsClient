...
class GetTechSourcesUseCase(private val newsRepository: NewsRepository) {
    suspend fun execute(category: String, source: String, page: Int): Resource<SourcesApiResponse> {
        return newsRepository.getAllTechSourcesCategory(category, source, page)
    }
}